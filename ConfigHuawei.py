...
#^M
# Title: Configura Roteadores v1.1^M
# Description: Python script to configuration some kind of ruters (yet not finished).^M
# Description: This script was developed with Brazilian Portuguese native speakers in mind.^M
#^M
# Configura Roteadores is distributed in the hope that it will be useful,^M
...
if __name__ == "__main__":
    ...
    try:
       ...
       print("JumpHost: ", ip_addr)
       report_file.write("Data: " + str(data_hoje) + "\n")
    except:
       print("\n\nFalha no acesso para o JumpHost\n")
       quit()
    ...
    with open(args.routers, "r") as file:
        ...
        for device_row in reader:
           ...
           try:
              net_connect.write_channel("ssh -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no " + 
                                         ssh_username  + "@"+device_row['device_ip']+'\n' )
              time.sleep(2)
           ...
           if((device_row['kind']).capitalize().strip() == "Cisco")  :
                   ...
                   Estado = device_row['device_ip'].split('-')
                   Huawei_Cmd_Set = [w.replace('$STATE$', Estado[1]) for w in Huawei_Cmd_Set]
                   output = net_connect.send_config_set((Huawei_Cmd_Set))
                   print("")
                   print("=============================================================")
                   print (output)
                   Huawei_Cmd_Set = [w.replace(Estado[1],'$STATE$') for w in Huawei_Cmd_Set]
                   print("=============================================================")
